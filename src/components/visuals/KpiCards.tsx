...
export function KpiCards({ items, active, reducedMotion, accentColor }: KpiCardsProps) {
  const container = {
    hidden: {},
    visible: {
      transition: {
    ...
    >
      {items.map((item, i) => (
        <motion.div key={`${item.value}-${item.suffix}-${i}`} variants={card}>
          <KpiCard item={item} accentColor={accentColor} active={active} reducedMotion={reducedMotion} />
        </motion.div>
      ))}
    </motion.div>
  );
...
function KpiCard({ item, accentColor, active, reducedMotion }: KpiCardProps) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl border border-white/[0.09] bg-gradient-to-br from-white/[0.06] via-white/[0.02] to-transparent px-5 py-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)] backdrop-blur-[1px] transition-[border-color,box-shadow] duration-300 hover:border-white/[0.14]"
      style={{
        ...
        <div className="min-w-0 flex-1">
          ...
          {typeof item.delta === 'number' && (
            <p className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold tabular-nums">
              ...
              >
                {item.delta >= 0 ? '▲' : '▼'} {formatBR(Math.abs(item.delta), 1)} {deltaUnit}
              </span>
