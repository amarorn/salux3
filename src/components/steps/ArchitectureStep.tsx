...
const LAYERS = [
  { icon: Brain, label: 'Raciocínio', desc: 'Orquestrador + LLMs', accent: '#7c3aed' },
  { icon: Wrench, label: 'Ferramentas', desc: 'Adapters tipados (EHR, RIS, LIS)', accent: '#06b6d4' },
  { icon: Database, label: 'Dados', desc: 'Feature store + memória vetorial', accent: '#10b981' },
  { icon: ShieldCheck, label: 'Governança', desc: 'Políticas, auditoria, kill switch', accent: '#f59e0b' },
];
        ...
        {minimal && step.content.bullets && step.content.bullets.length > 0 && (
          <motion.div variants={item} className="grid gap-3 sm:grid-cols-2">
            {step.content.bullets.map((line, i) => (
              ...
              >
                <p className="text-[13px] leading-snug text-slate-100">{line}</p>
              </motion.div>
            ))}
          </motion.div>
        )}
